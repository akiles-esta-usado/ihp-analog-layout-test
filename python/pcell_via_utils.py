...
def __get_viastack_minseperation(
    pdk: MappedPDK, viastack: Component, ordered_layer_info
) -> tuple[float, float]:
    """internal use: return absolute via separation and top_enclosure (top via to top met enclosure)"""
    get_sep = lambda _pdk, rule, _lay_, comp: (
        rule + 2 * comp.extract(layers=[_pdk.get_glayer(_lay_)]).xmax
    )
    ...
    ****NOTE: generator can figure out which layer is top and which is bottom (i.e. met5 is higher than met1)
    ...
    if level1 == level2:
        ...
        for level in range(level1, level2 + 1):
            ...
            # get layer sizing
            mode = (
                "below" if level == level2 else ("above" if level == level1 else "both")
            )
...
def via_array(
    ...
    ****NOTE: this implies lay_bottom
    ...
    via_abs_spacing, top_enclosure = __get_viastack_minseperation(
        pdk, viastack, ordered_layer_info
    )
    ...
    if lay_every_layer:
        for i in range(level1 + 1, level2):
            ...
            viaarray << rectangle(
                size=bdims, layer=pdk.get_glayer(f"met{i}"), centered=True
            )
